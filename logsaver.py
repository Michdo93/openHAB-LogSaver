...
class OpenHABLogReader(object):
    ...
    def createTable(self, tablename):
        try:
            self.cursor.execute(f"CREATE TABLE IF NOT EXISTS {tablename} (id INT NOT NULL AUTO_INCREMENT, datetime DATETIME, log_level VARCHAR(10), log_event VARCHAR(30), log_message LONGTEXT, CONSTRAINT AvoidTwiceLogsConstraint UNIQUE (datetime,log_level,log_event,log_message), PRIMARY KEY(id)) ENGINE=InnoDB DEFAULT CHARSET=latin1")
        except mariadb.Error as e:
            ...
            if filename == file:
                while True:
                    ...
                    if self.stamp > self._cached_stamp:
                        ...
                        log_event = splitted[3].rsplit(".", 1)[-1].replace("]", "")
                        log_message = ""
...
if __name__ == "__main__":
    ...
    reader.connect("", "", "", 3306, "OpenHAB_LOGS")
